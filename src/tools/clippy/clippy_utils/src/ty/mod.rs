...
pub fn make_normalized_projection<'tcx>(
    ...
    fn helper<'tcx>(tcx: TyCtxt<'tcx>, typing_env: ty::TypingEnv<'tcx>, ty: AliasTy<'tcx>) -> Option<Ty<'tcx>> {
        ...
        match tcx.try_normalize_erasing_regions(
            typing_env,
            Unnormalized::new_wip(Ty::new_projection_from_args(tcx, ty::IsRigid::No, ty.kind.def_id(), ty.args)),
        ) {
            Ok(ty) => Some(ty),
            Err(e) => {
...
pub fn make_normalized_projection_with_regions<'tcx>(
    ...
    fn helper<'tcx>(tcx: TyCtxt<'tcx>, typing_env: ty::TypingEnv<'tcx>, ty: AliasTy<'tcx>) -> Option<Ty<'tcx>> {
        ...
        match infcx
            .at(&cause, param_env)
            .query_normalize(Ty::new_projection_from_args(tcx, ty::IsRigid::No, ty.kind.def_id(), ty.args))
        {
            Ok(ty) => Some(ty.value),
            Err(e) => {
                debug_assert!(false, "failed to normalize type `{ty}`: {e:#?}");
