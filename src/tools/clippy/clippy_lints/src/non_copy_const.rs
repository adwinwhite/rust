...
impl<'tcx> NonCopyConst<'tcx> {
    ...
    fn is_init_expr_freeze(
        ...
        let ty = EarlyBinder::bind(typeck.expr_ty(e)).instantiate(tcx, gen_args).skip_norm_wip();
        let ty = tcx.try_normalize_erasing_regions(typing_env, Unnormalized::new_wip(ty)).unwrap_or(ty);
        match self.is_ty_freeze(tcx, typing_env, ty) {
            IsFreeze::Yes => true,
            IsFreeze::No => false,
            ...
            loop {
                match init_expr.kind {
                    ...
                    ExprKind::Path(ref init_path) => {
                        let next_init_args =
                            EarlyBinder::bind(init_typeck.node_args(init_expr.hir_id)).instantiate(tcx, init_args).skip_norm_wip();
