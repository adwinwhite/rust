    ...
    match (source_ty.kind(), target_ty.kind()) {
        ...
        (&ty::Adt(source_adt_def, source_args), &ty::Adt(target_adt_def, target_args)) => {
            ...
            let source_field = tcx.normalize_erasing_regions(
                typing_env,
                Unnormalized::new(coerce_field.ty(*tcx, source_args)),
            );
            let target_field = tcx.normalize_erasing_regions(
                typing_env,
                Unnormalized::new(coerce_field.ty(*tcx, target_args)),
            );
...
impl<'v> RootCollector<'_, 'v> {
    ...
    fn push_extra_entry_roots(&mut self) {
        ...
        let main_ret_ty = self.tcx.normalize_erasing_regions(
            ty::TypingEnv::fully_monomorphized(),
            Unnormalized::new(main_ret_ty.no_bound_vars().unwrap()),
        );
