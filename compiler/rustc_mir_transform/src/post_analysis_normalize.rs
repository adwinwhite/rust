...
impl<'tcx> MutVisitor<'tcx> for PostAnalysisNormalizeVisitor<'tcx> {
    ...
    fn visit_const_operand(&mut self, constant: &mut ConstOperand<'tcx>, location: Location) {
        ...
        if let Ok(c) = self
            .tcx
            .try_normalize_erasing_regions(self.typing_env, Unnormalized::new(constant.const_))
        {
            constant.const_ = c;
        }
    ...
    fn visit_ty(&mut self, ty: &mut Ty<'tcx>, _: TyContext) {
        ...
        // see #91745
        if let Ok(t) =
            self.tcx.try_normalize_erasing_regions(self.typing_env, Unnormalized::new(*ty))
        {
            *ty = t;
        }
    }
