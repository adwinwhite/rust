...
impl<'body, 'a, 'tcx> VnState<'body, 'a, 'tcx> {
    ...
    fn pointers_have_same_metadata(&self, left_ptr_ty: Ty<'tcx>, right_ptr_ty: Ty<'tcx>) -> bool {
        ...
        if left_meta_ty == right_meta_ty {
            ...
            .tcx
            .try_normalize_erasing_regions(self.typing_env(), Unnormalized::new(left_meta_ty))
            && let Ok(right) = self
                .tcx
                .try_normalize_erasing_regions(self.typing_env(), Unnormalized::new(right_meta_ty))
        {
            left == right
        } else {
            false
