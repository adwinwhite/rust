...
declare_lint! {
    ...
    ///  --> counter_examples/cache-depth.rs:12:20
    ///   |
    ///   |     require_sync::<Foo<Foo<Foo<Foo<Foo<Foo<()>>>>>>>();
    ///   |                    ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
    ///   |
    ...
    ///  --> counter_examples/cache-depth.rs:9:20
    ///   |
    ///   | fn require_sync<T: Sync>() {}
    ...
    /// They also use caches to avoid redundant computation. This is a performance optmization and
