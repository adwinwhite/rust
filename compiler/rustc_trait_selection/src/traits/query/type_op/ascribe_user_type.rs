...
fn relate_mir_and_user_args<'tcx>(
    ...
    let args = if is_inherent_assoc_const {
        ...
        let user_self_ty = ocx.normalize(&cause, param_env, Unnormalized::new(args[0].expect_ty()));
    ...
    let ty = ocx.normalize(&cause, param_env, Unnormalized::new(ty));
    ...
    for (instantiated_predicate, predicate_span) in instantiated_predicates {
        ...
        let instantiated_predicate =
            ocx.normalize(&cause, param_env, Unnormalized::new(instantiated_predicate));
    ...
    if let Some(UserSelfTy { impl_def_id, self_ty }) = user_self_ty {
        ...
        let self_ty = ocx.normalize(&cause, param_env, Unnormalized::new(self_ty));
        let impl_self_ty = tcx.type_of(impl_def_id).instantiate(tcx, args).skip_normalization();
        let impl_self_ty = ocx.normalize(&cause, param_env, Unnormalized::new(impl_self_ty));
