...
impl<'a, 'tcx> TypeErrCtxt<'a, 'tcx> {
    ...
    fn report_host_effect_error(
        ...
        *diag.long_ty_path() = file;
        ...
        if let Ok(Some(ImplSource::UserDefined(impl_data))) =
            self.enter_forall_skipping_norm(trait_ref, |trait_ref_for_select| {
    ...
    fn emit_specialized_closure_kind_error(
        ...
        if self.enter_forall_skipping_norm(found_args, |found_args| {
            self.enter_forall_skipping_norm(expected_args, |expected_args| {
...
impl<'a, 'tcx> TypeErrCtxt<'a, 'tcx> {
    ...
    pub(super) fn error_implies(
        ...
        if let Some(error) = error.predicate.as_trait_clause() {
            self.enter_forall_skipping_norm(error, |error| {
    ...
    pub(super) fn report_similar_impl_candidates(
        ...
        if let [single] = &impl_candidates {
            ...
            if self.probe(|_| {
                ...
                self.enter_forall_skipping_norm(trait_pred, |obligation_trait_ref| {
