        ...
        match (expected_inner.kind(), found_inner.kind()) {
            (ty::FnPtr(sig_tys, hdr), ty::FnDef(did, args)) => {
                ...
                let found_sig = &(self.normalize_fn_sig)(
                    self.tcx.fn_sig(*did).instantiate(self.tcx, args).skip_norm_wip(),
                );
            ...
            (ty::FnDef(did1, args1), ty::FnDef(did2, args2)) => {
                let expected_sig = &(self.normalize_fn_sig)(
                    self.tcx.fn_sig(*did1).instantiate(self.tcx, args1).skip_norm_wip(),
                );
                let found_sig = &(self.normalize_fn_sig)(
                    self.tcx.fn_sig(*did2).instantiate(self.tcx, args2).skip_norm_wip(),
                );
